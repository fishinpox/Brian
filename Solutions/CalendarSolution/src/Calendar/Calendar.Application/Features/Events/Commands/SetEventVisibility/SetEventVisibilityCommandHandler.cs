...
public class SetEventVisibilityCommandHandler(ICalendarDbContext db, ICurrentUserService currentUser)
    : IRequestHandler<SetEventVisibilityCommand, Result>
{
    public async Task<Result> Handle(SetEventVisibilityCommand request, CancellationToken cancellationToken)
    {
        ...
        personalEvent.SetVisibility(request.IsVisible);
