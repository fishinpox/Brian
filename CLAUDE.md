...
Solutions/
  ...
  AgencySolution/           ← Agency.{Domain,Application,Infrastructure,API} (CRM contact/company records), :7005
...
- **Central Package Management**: each service solution manages its own NuGet versions in its own `Directory.Packages.props` (there is no single shared one) — never add `Version=` attributes to `.csproj` files. Notable pinned versions: `Mapster 10.0.0` + `Mapster.DependencyInjection 10.0.0` (not 7.x), `Scalar.AspNetCore 2.0.0` (requires explicit `using Scalar.AspNetCore;`), `Refit 13.1.0` (bumped from the vulnerable 7.1.2 across every service — see the Refit note below for the source-generator caveat this version requires).
...
- **Moderation.API port collision fixed**: `Moderation.API` moved from `:7012`/`:5012` to `:7014`/`:5014` — it collided with `Onboarding.API`, which also defaulted to `:7012`/`:5012` and is the one referenced by `Onboarding:BaseUrl` (Identity.API) and `ONBOARDING_LOGIN_URL` (Calendar.Web), so Moderation was the lower-risk service to move since nothing hardcodes its port. Landed on `:7014` rather than `:7013` since the new `Chat.API` (see above) claimed `:7013` first.
- **Agency CRM added** (`Documentation/CRM/CRM.md`): built out the previously scaffold-only `AgencySolution` into a working `Contact`/`Company` CRUD backend (`/api/agency/contacts`, `/api/agency/companies`, `:7005`, Gateway route already existed, unused until now), gated by the existing `UserRole.AgencyAdmin` — no new role needed. [AFFiNE](https://affine.pro) (named in the original spec) was ruled out after research: it's a Notion/Miro-style notes+whiteboard workspace with no contact/deal objects, not an actual CRM. Scope is deliberately minimal per explicit user sign-off: contact/company records only (no deals/pipeline, no notes/activity history, no tasks/reminders yet), standalone (no linkage to Identity's Creator/Profile records yet), roster-first-then-sponsors reflected via a `ContactCategory`/`CompanyCategory` enum (`Roster`/`Sponsor`/`Other`) rather than separate entities. Hard-delete, not soft-delete — confirmed no entity anywhere in the repo uses a soft-delete pattern (`WallpaperItem.IsActive` is an availability flag, not a delete mechanism). **Gotcha worth knowing for the next service that adds role-gated endpoints**: `TokenService` emits role grants under a custom `"roles"` JWT claim type, but ASP.NET Core's default JWT-bearer claim mapping auto-renames it to the standard role claim type on validation — so `[Authorize(Roles = "...")]` works with **zero extra config**. Explicitly setting `TokenValidationParameters.RoleClaimType = "roles"` (the seemingly-obvious fix) is wrong and silently breaks it, since the raw `"roles"`-typed claim no longer exists post-remapping — confirmed by a temporary debug claims-dump endpoint before landing on the real fix (leave `RoleClaimType` unset). This is the first service in the repo to use role-based `[Authorize]` at all, so this had no prior precedent to copy.
...
- **Phase 3** — Analytics, SignalR Redis scale-out — scaffolded only (Agency is no longer scaffold-only — see above)
